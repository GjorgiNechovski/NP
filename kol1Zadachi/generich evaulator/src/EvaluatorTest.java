...
class EvaluatorBuilder{
    ...
    static <T extends Comparable<T>> IEvaultaor<T> build(String opetator){
        switch (opetator){
            case ">":{
                return (a,b) -> a.compareTo(b)>0;
            }
            case "<":{
                return (a,b) -> b.compareTo(a)>0;
            }
            case "==":{
                return (a,b)->a.compareTo(b)==0;
            }
            ...
            default:
                throw new IllegalStateException("Unexpected value: " + opetator);
        }
    }
}
...
class Evaluator<T> {
    static <T extends Comparable<T>> boolean evaluateExpression (T left, T right, String operator){
        IEvaultaor<T> result = EvaluatorBuilder.build(operator);
