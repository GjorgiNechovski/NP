...
class LogSystem<T extends ILog & Comparable<T>> {
    ...
    void printResults() {
        ...
        LogProcessor<T> firstLogProcessor = (elements) -> elements.stream().filter(x->x.getType().equals("INFO")).collect(Collectors.toCollection(ArrayList::new));

        LogProcessor<T> secondLogProcessor = (elements)-> elements.stream().filter(x->x.getMessage().length()<100).collect(Collectors.toCollection(ArrayList::new));

        LogProcessor<T> thirdLogProcessor = (elements) ->elements.stream().sorted().collect(Collectors.toCollection(ArrayList::new));
