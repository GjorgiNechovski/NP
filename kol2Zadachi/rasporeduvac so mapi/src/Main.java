...
class Sheduler<T> {
    TreeMap<Date, T> elements = new TreeMap<>();

    public Sheduler(Date date, T t) {
        elements.put(date,t);
    }
    ...
    public boolean remove(Date t){
        T temp = elements.get(t);
        return temp != null;
    }

    public T next(){
        return elements.get(elements.ceilingKey(new Date()));
    }

    public T last(){
        return elements.get(elements.floorKey(new Date()));
    }

    public ArrayList<T> getAll(Date begin,Date end){
        return elements.navigableKeySet().
    }
